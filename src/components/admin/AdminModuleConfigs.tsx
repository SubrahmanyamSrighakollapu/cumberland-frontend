...
export function AdminRoomsPage() {
  ...
  const rows = rooms.map((r: any) => ({
    ...
    featured: r.is_featured || (r.name.includes("Queen") || r.name.includes("Twin") || r.name.includes("Family")),
...
export function AdminDiningPage() {
  ...
  const config: ModuleConfig<any> = {
    title: "Eat & Drink",
    description:
      "Curate local cafés, restaurants and Hunter Valley bistros. Highlight featured venues, pricing tiers and proximity to the motel.",
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.75}>
...
export function AdminActivitiesPage() {
  ...
  const config: ModuleConfig<any> = {
    title: "Things to Do",
    description:
      "Create and manage activities, adventures and local attractions. Use categories to help guests easily filter by wine, nature, family or culture.",
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.75}>
