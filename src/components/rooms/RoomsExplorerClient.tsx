...
import { RoomDetail, apiRoomToDetail } from "@/utils/roomDataClient";

export interface RoomsExplorerClientProps {
  initialRooms: RoomDetail[];
}

export default function RoomsExplorerClient({
  initialRooms,
}: RoomsExplorerClientProps) {
  const [apiRooms, setApiRooms] = useState<RoomDetail[] | null>(
    initialRooms && initialRooms.length > 0 ? initialRooms : null
  );
  ...
  const display: RoomDetail[] = useMemo(() => {
    return apiRooms ?? initialRooms ?? [];
  }, [apiRooms, initialRooms]);
