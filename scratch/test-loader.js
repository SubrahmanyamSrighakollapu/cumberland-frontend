const { getPublishedRoomBySlugServer, getAllPublishedRoomsServer, BackendUnavailableError } = require("../src/utils/publicDataLoader");
...
async function runLoaderTests() {
  ...
  try {
    ...
    console.log(`[TEST 1] Published room fetch -> Slug: ${room?.slug || "null"}, Name: ${room?.name || "null"}`);
  } catch (e) {
  ...
  try {
    ...
    console.log(`[TEST 2] Missing/Unpublished room fetch -> Result: ${room === null ? "SUCCESS (Returned null for 404)" : room}`);
  } catch (e) {
  ...
  try {
    ...
    console.log(`[TEST 3] Paginated rooms list fetch -> Total published rooms retrieved: ${rooms.length}`);
  } catch (e) {
  ...
  try {
    // Force invalid backend URL to verify BackendUnavailableError is thrown
    ...
    if (e.name === "BackendUnavailableError" || e instanceof BackendUnavailableError) {
      console.log(`[TEST 4] Backend outage simulation -> SUCCESS (Threw distinct error: ${e.message})`);
    } else {
